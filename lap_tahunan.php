...
if (isset($_GET['download'])) {
    ...
    header("Content-Disposition: attachment; filename=laporan_tahunan.csv");

    $output = fopen("php://output", "w");
    fputs($output, "\xEF\xBB\xBF");
    fputcsv($output, ["No", "Tahun", "Total Produk Terjual", "Pendapatan"], ";");

    $sql = "SELECT * FROM laporan_tahunan ORDER BY id DESC";
...
if (isset($_GET['download_excel'])) {
    ...
    $sheet->setCellValue('C1', 'Total Produk Terjual');
    $sheet->setCellValue('D1', 'Pendapatan');
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $sheet->setCellValue("C$rowNum", $row['total_produk_terjual']);
        $sheet->setCellValue("D$rowNum", $row['pendapatan']);
        $no++;
        $rowNum++;
    }
...
<div class="container mt-5">
    <h2 class="mb-4" style="color: white">Laporan Tahunan Penjualan</h2>
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        echo "<tr>
                            ...
                            <td>
                                <a href='detail_lap_tahunan.php?id=" . intval($row['id']) . "' class='btn btn-primary btn-sm'>Detail</a>
                            </td>
                        </tr>";
                        $no++;
                    }
